...
spark = SparkSession \
    .builder \
    .appName("ap-coverage-events") \
    .getOrCreate()

def test_jobs():
    from analytics.utils.time_util import current_epoch_seconds
    from analytics.jobs.utils import *
    start_time = current_epoch_seconds()//(3600*24) * (3600*24) - (3600*2) * 1
    end_time = start_time + 3600
    print(start_time, end_time)
    env = 'production'
    # env = 'staging'

    agg_job = stats_aggregator_job(start_time, end_time, "ap-events",  spark, test_env=env, debug_mode=False, input_bucket_env=env)
    agg_job.execute()

    enrich_job= data_enrichment_job("ap_coverage_enrichment",  start_time, end_time, spark=spark, test_env=env)
    enrich_job.execute()

    detection_job = start_debug_job('ap_coverage_detection', start_time, end_time, spark=spark, test_env=env)
    data = run_category_transform(detection_job, 'all')
    data.printSchema()
    gen = get_event_generator(detection_job, 'all', 'APCoverageEvent')
...
def test_detection():
    ...
    data.filter('ap1_avg_nclients>0').select(cols).show()
...
def test_recommender():
    ...
    job = entity_suggestion_job(start_time, end_time, spark=spark, test_env='staging', debug_mode=True)
    # job = entity_suggestion_job(start_time, end_time, spark=spark, debug_mode=False)
    ...
    recommender.compose_email_content(new_suggestions)
