...
bar_df = pd.DataFrame(columns=["as_number", "dd_data_pr~-15m", "dd_data_pr~+15m"])
...
def process_event( asn_dict, org_id, site_id, entity_type, event_name, event_type ):
    global tot_records, result_df, bar_df
    if( (org_id, site_id) in asn_dict ):
        ...
        for n in range( len( asns ) ):
            ...
            starttime = resp['hits']['hits'][i]['_source']['start_time'] / 1000  # round to milliseconds
            dd_starttime = starttime - 900 # back 15 mins
            dd_endtime = starttime + 900 # forward 15 mins
            ...
            dataline = {
                "datetime": datetime.fromtimestamp(round(starttime), utctz).strftime('%d/%m/%y %H:%M'),
                "org_id": org_id,
                "site_id": site_id,
                ...
                "dd_data_pr~-15m": ddresp.json()[0]['total'],
                "dd_data_pr~+15m": ddresp.json()[len(ddresp)-1]['total']
                }
            result_df = result_df.append(dataline, ignore_index = True)
            ddresparr = ddresp,json()
            bar_row = [asn, ddresparr[0]['total'], ddresparr[len(ddresparr)-1]['total'], calc_median( ddresparr )]
